...
import { recordActivityEvents, recordEventFilters } from "../copy/proof-record";
...
export function ProofRecordTimeline({ entries, finalizedAt, filter, onFilter, onSelect }: {
  ...
  const visible = ordered.filter(entry => !detailed || selected === "ALL" || entry.category === selected);
  return <View style={styles.root} accessibilityLabel="Proof activity">
    ...
    {!visible.length ? <View style={styles.empty}>
      ...
      const title = humanChronologyTitle(entry.eventType, entry.title);
...
const styles = StyleSheet.create({
  root: { gap: 0 }, modeRow: { flexDirection: "row", gap: 20, marginBottom: 16 }, modeButton: { minHeight: 44, justifyContent: "center" }, filters: { flexDirection: "row", gap: 8, paddingBottom: 16 },
  ...
  card: { borderRadius: 10, padding: 10, gap: 8, minHeight: 44 }, cardTop: { flexDirection: "row", gap: 6, alignItems: "center", flexWrap: "wrap" },
