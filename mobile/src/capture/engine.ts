...
import type { IdentifierPolicy } from '../../../backend/src/identifiers/types';
import { identifierCaptureEnabled } from './identifier-observation';
import { captureIdentifiers, identifierFailureBlocks } from './identifier-storage';
...
export async function sealNativeEngine(client:PackProofV2Client,capture:LocalCapture):Promise<void> {
  ...
  if (identifierCaptureEnabled(capture.identifierPolicy)) {
    const identifiers = await captureIdentifiers(client, capture).catch(error => { if (identifierFailureBlocks(error)) throw error; return null; });
    for (const row of identifiers?.journal.snapshot().review?.observations ?? []) {
      if (row.route !== 'SHIPPING' || row.decision?.decision === 'NOT_THIS_SHIPMENT') continue;
      const value = row.observation.rawText.replace(/[\s-]/g, '').toUpperCase();
