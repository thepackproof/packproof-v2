...
import { getAttestationAvailability, getAttestationMethod, prepareAttestationKey } from "../attestation/native";
...
export async function capturePreflight(client: PackProofV2Client, userId: string, sellerAttestation = true): Promise<void> {
  ...
  const capabilities = requireCaptureCapabilities(response, sellerAttestation, getAttestationMethod());
  ...
  if (sellerAttestation) {
    ...
    if (!available.available) throw Object.assign(new Error(available.message || "Set up supported biometrics in your device Settings before recording."), { code: available.code });
