...
export type CapturePhase = "RECORDING" | "LOCAL_ONLY" | "UPLOAD_QUEUED" | "UPLOADING" | "BYTES_RECEIVED" | "PRESERVATION_PENDING" | "CONFIRMATION_NEEDED" | "FINALIZATION_PENDING" | "SUBMITTED" | "FINALIZED" | "NEEDS_SIGN_IN" | "NEEDS_ATTENTION" | "DISCARD_PENDING";
...
export interface CaptureRecoveryState {
  ...
  completionNotificationHandled?: boolean;
...
export function captureRecoveryLabel(phase: CapturePhase): string {
  switch (phase) {
    ...
    case "DISCARD_PENDING": return "Discard will finish when PackProof can close the incomplete upload.";
  }
}
...
export function recoveryRetry(error: unknown, attempt: number, now = Date.now(), random = Math.random()): { retryable: boolean; phase: CapturePhase; code: string; message: string; nextRetryAt: number | null } {
  ...
  const intervention = /BIOMETRIC|ATTESTATION_|IDENTIFIER_.*(?:REVIEW|INTEGRITY)|CAPTURE_SESSION_EXPIRED|CAPTURE_ORIGINAL|CAPTURE_LOCAL_FILE_MISSING|CAPTURE_DISCARD_PENDING|CAPABILITY|STORAGE|PROOF_NOT_READY|FULFILLMENT_CAPTURE_REQUIRED/.test(code);
