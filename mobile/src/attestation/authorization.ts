...
export type NativeAttestationMethod = "ANDROID_BIOMETRIC_STRONG" | "IOS_BIOMETRIC";
export function isNativeAttestationMethod(method: unknown): method is NativeAttestationMethod {
  return method === "ANDROID_BIOMETRIC_STRONG" || method === "IOS_BIOMETRIC";
}

export function validateSellerChallenge(challenge: { challengeId: string; payload: string; expiresAt: string }, expected: {
  proofId: string; userId: string; captureSessionId: string; sha256: string; publicKey: string; captureManifestSha256?:string; method?: NativeAttestationMethod;
}, now = Date.now()): void {
  ...
  if (!payload || payload.version !== 1 || payload.method !== (expected.method ?? "ANDROID_BIOMETRIC_STRONG") ||
...
export function recordedSellerAuthorization(proof: ProofView, evidenceId: string | undefined, userId: string): SellerAttestationAuthorization | null {
  ...
  const row = proof.attestations?.find(item => item.attestedBy === userId && item.relatedEvidenceId === evidenceId &&
    ...
    isNativeAttestationMethod(item.authorization.method));
