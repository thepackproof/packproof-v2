...
- `image-size` stays on **1.2.1**, the callable synchronous API used by Metro.
  Upstream **2.0.2** is still affected by both current advisories; neither advisory
  identifies a patched release. Its major API also removes synchronous file use.
  ...
  [Maintainer releases](https://github.com/image-size/image-size/releases).
- A checked installation guard rejects ICNS, JXL container, and ISO-BMFF image
  ...
  do not bypass the guard. This is a local mitigation, not an upstream fix.
...
source; dependency drift stops installation for review. Repeated installation is
...
- Offline Android JavaScript/Hermes export passed: 778 bundled modules and 20
...
- Four build dependency tests pass: patch integrity/idempotency; malformed
...
- Registry audit after the tar update reports **6 high, 0 critical** findings,
  down from **10 total including 1 critical**. The six remaining entries are
  `image-size` and its Metro / React Native dependency cascade. Audit findings
  are retained transparently; the signature guard is not represented as a clean
  upstream audit.
