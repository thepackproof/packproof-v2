...
Verifier 1.3.0 additionally recognizes a stored signed lifecycle snapshot at `lifecycle/snapshot.json`, declared by matching `{path,sha256,snapshotId}` descriptors in the package and archive sources. Its signature binds the original root, transaction/tenant context, exact received supplement head and sealed-stage inventory. `--expected-proof-id`, `--expected-tenant-id` and `--expected-lifecycle-snapshot-sha256` accept independently known required context; an absent or different required binding fails closed. Without an expected snapshot digest, valid older snapshots remain historical received snapshots, never proof of currentness. Results expose `lifecycleSnapshot` separately from the root and always retain unknown offline freshness and unavailable current revocation knowledge. Nonempty derivative inventories are rejected until a supported signed derivation profile exists. See [the frozen lifecycle format](../docs/future-platform/PORTABLE_FORMAT.md) for the complete extension and bounded verification meanings.
