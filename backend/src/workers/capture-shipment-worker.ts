...
async function ensureConnection(db: Database, clock: Clock, transactionId: string, actor: string, reference?: string) {
  await db.transaction(async tx => {
    ...
    const own = (await tx.query<{id:string}>("SELECT id FROM integration_connections WHERE owner_user_id=$1 AND adapter_key=$2 AND status='ACTIVE' ORDER BY created_at,id LIMIT 2",[actor,EASYPOST_TRACKER_ADAPTER_KEY])).rows;
    let connectionId: string;
    if (own.length === 1) connectionId = own[0].id;
    else if (own.length === 0 && reference && easypostCredentialReferenceAllowed(reference)) {
      connectionId = `conn_capture_${sha256Hex(actor).slice(0,32)}`;
      await tx.query(`INSERT INTO integration_connections(id,owner_user_id,adapter_key,provider,credential_reference,status,created_at,updated_at)
        VALUES($1,$2,$3,'easypost',$4,'ACTIVE',$5,$5) ON CONFLICT(id) DO NOTHING`,[connectionId,actor,EASYPOST_TRACKER_ADAPTER_KEY,reference,clock.now().toISOString()]);
...
export async function dispatchCaptureShipments(db: Database, clock: Clock, deps: CaptureShipmentDependencies, limit = 5) {
  ...
  for (let index = 0; index < Math.min(Math.max(limit,1),25); index++) {
    ...
    if (!job) break;
    try {
      await ensureConnection(db,clock,job.transaction_id,job.actor_user_id,deps.defaultEasyPostCredentialReference);
      const result = await executeTrustedShipmentSync(db,clock,job.actor_user_id,job.transaction_id,deps);
      const terminal = result.events.some(e=>['DELIVERED','RETURN_TO_SENDER','CANCELLED'].includes(e.eventType));
      ...
      const waiting = ['INTEGRATION_NOT_FOUND','INTEGRATION_CREDENTIALS_UNAVAILABLE','INTEGRATION_NEEDS_REAUTH','INTEGRATION_DISABLED','PROVIDER_AUTH_FAILED'].includes(code);
