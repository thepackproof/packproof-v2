...
export async function queueShippoNotification(db:Database,clock:Clock,raw:Buffer) {
  ...
  const rows=(await db.query<{transaction_id:string;connection_id:string;carrier:string|null;provider_mode:string|null}>(`SELECT j.transaction_id,c.id AS connection_id,COALESCE(ta.carrier,s.carrier) AS carrier,j.provider_mode FROM capture_shipment_jobs j JOIN transaction_shipment_connections b ON b.transaction_id=j.transaction_id JOIN integration_connections c ON c.id=b.connection_id LEFT JOIN transaction_shipping s ON s.transaction_id=j.transaction_id LEFT JOIN proof_tracking_associations ta ON ta.transaction_id=j.transaction_id WHERE c.adapter_key='shippo-tracker' AND c.status='ACTIVE' AND COALESCE(ta.tracking_number,s.tracking_number)=$1 AND j.next_run_at IS NOT NULL LIMIT 100`,[tracking])).rows;
