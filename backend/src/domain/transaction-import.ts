...
export async function importNormalizedTransaction(
  ...
  return db.transaction(async (tx) => {
    if (parsed.provider === "ebay" && parsed.provenance.source === "MARKETPLACE_API") {
      await lockEbayPrivacy(tx);
      const environment = parsed.externalAccountReference?.startsWith("production") ? "production" : "sandbox";
      if (await isEbaySubjectSuppressed(tx, environment, parsed.buyer?.externalId, parsed.buyer?.displayName)) {
        throw new DomainError("EBAY_ACCOUNT_DELETION_PENDING", "This eBay order has an unresolved account deletion request", 409);
      }
    }
