...
export async function evaluateContinuity(
  ...
  const algorithmVersion = input.algorithmVersion?.trim() || CONTINUITY_ALGORITHM_V1;
  ...
  return db.transaction(async (tx) => {
    ...
    const result = input.finding == null || input.finding === ""
      ? derived
      : requireContinuityResult(input.finding);
    const summary = continuitySummary(result);
    const id = newId("cev");
    const now = clock.now();
    try {
      await tx.query(
...
function deriveResult(
  ...
  const complete = pairs.filter((pair) => pair.originEvidenceId && pair.receivedEvidenceId);
  if (complete.length === 0) {
    return "INCONCLUSIVE";
  }
  if (complete.length < pairs.length) {
    return "INCONCLUSIVE";
  }
  return "CONSISTENT";
}
