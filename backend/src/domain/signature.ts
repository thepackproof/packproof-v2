...
export function renderSignatureCaseHtml(preview: SignatureCasePreview, previewSha256: string, approver: string, webBaseUrl = "https://thepackproof.com") {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><title>PackProof · ${esc(preview.title)}</title><style>
    body{font:15px/1.6 system-ui,sans-serif;color:#14283e;background:#fff;margin:0}main{max-width:920px;margin:40px auto;padding:0 32px}header{border-left:2px solid #2588d5;padding-left:20px;border-bottom:1px solid #dbe4ec;padding:24px 20px}.brand{font-weight:800;color:#1766ac;letter-spacing:.05em}h1{font-family:Georgia,serif;font-size:30px;line-height:1.2}h2{font-size:20px;margin-top:30px;color:#125b90}table{border-collapse:collapse;width:100%;table-layout:fixed}td,th{padding:10px;border:1px solid #dbe4ec;text-align:left;vertical-align:top;overflow-wrap:anywhere}th{background:#f2f7fb}.hash{font:12px/1.5 monospace;overflow-wrap:anywhere}.source{color:#496579;font-size:13px}blockquote{margin:12px 0;padding:12px 18px;border-left:3px solid #32a58b;background:#f4faf8;white-space:pre-wrap}.record-line{border-left:2px solid #2588d5;padding-left:20px}.record-line.shipment{border-color:#218d85}.record-line.statement{border-color:#8465b0}.fine-print{font-size:12px;line-height:1.5;color:#496579}.fine-print h2{font-size:15px}.meta{overflow-wrap:anywhere;font-size:12px;color:#496579}@media print{main{margin:0;padding:0}h2,tr,blockquote{break-inside:avoid}body{font-size:11pt}}
    ...
    ${preview.shipping ? `<section class="record-line shipment"><h2>Reported shipping events</h2>${list(preview.shipping.events.map(e => `${e.occurredAt} · ${e.title} · Source: ${e.source}${e.provider ? ` / ${e.provider}` : ""} · Event ${e.eventId}`))}</section>` : ""}
    ...
    ${preview.statements.length ? `<section class="record-line statement"><h2>Participant statements</h2>${list(preview.statements.map(s => `${s.statement} · Source: ${s.source} · ${s.attestationId}`))}</section>` : ""}
    ...
    <section class="fine-print"><h2>Gaps and limits</h2>${list(preview.gaps)}${list(preview.limitations)}</section><footer class="meta">This packet contains the exact approved snapshot selection. It does not submit a claim or grant additional access.</footer></main></body></html>`;
}
