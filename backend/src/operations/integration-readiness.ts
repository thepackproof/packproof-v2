...
export function integrationReadiness(env:NodeJS.ProcessEnv=process.env) {
  ...
  check('ebay',[
    ...
    [/^[A-Za-z0-9_-]{32,80}$/.test(env.PACKPROOF_EBAY_DELETION_VERIFICATION_TOKEN??''),'Configure account-deletion endpoint verification'],
    [env.PACKPROOF_EBAY_DELETION_ERASURE_VALIDATED==='true','Complete and validate eBay personal-data erasure, including preserved copies'],
