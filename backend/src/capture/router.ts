...
import { issueIntent,bindIntent,sealCapture,receiveBatch,captureStatus,captureCapsules,recoverIntentContext,asDomainError } from './service.js';
...
export function captureEngineRouter(deps:AppDependencies){
 ...
 router.get('/capture-intents/:intentId/context',route(async(r,s)=>{s.json(await recoverIntentContext(deps.db,deps.clock,actor(r),r.params.intentId));}));
