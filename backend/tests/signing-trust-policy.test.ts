...
const registry: SigningTrustRegistry = { version: 1, domain: "PACKPROOF_SIGNING_TRUST_REGISTRY", publishedAt: "2026-01-09T00:00:00.000Z", nextReviewAt: "2026-02-01T00:00:00.000Z", keys: [{ keyId: "manifest-key", algorithm: "ECDSA_SHA_256", publicKeyPem: pem(key.publicKey), status: "ACTIVE", validFrom: "2025-01-01T00:00:00.000Z", validUntil: null, statusEffectiveAt: null, reason: null }] };
...
describe("dated signing trust policy", () => {
  it("verifies rotated historical keys only using an independently trusted registry", () => {
    ...
    expect(verify({ ...registry, keys: [{ ...registry.keys[0], status: "RETIRED", statusEffectiveAt: "2026-01-08T00:00:00.000Z" }] }).historicalTrust).toBe("TRUSTED_AT_REGISTRY_SNAPSHOT");
  ...
  it("distinguishes compromised, revoked, unknown and stale trust from mathematical verification", () => {
    const compromised = verify({ ...registry, keys: [{ ...registry.keys[0], status: "COMPROMISED", statusEffectiveAt: "2026-01-08T00:00:00.000Z", reason: "Test compromise" }] });
    ...
    expect(verify({ ...registry, keys: [{ ...registry.keys[0], status: "REVOKED", statusEffectiveAt: "2026-01-08T00:00:00.000Z" }] }).fullyVerified).toBe(false);
