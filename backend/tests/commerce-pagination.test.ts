...
describe("bounded commerce cursor progress", () => {
  ...
  it("resumes from completed pages after the batch limit and a provider failure", async () => {
    ...
    expect(first.body).toMatchObject({ discoveredCount: MAX_COMMERCE_SYNC_PAGES, cursor: String(MAX_COMMERCE_SYNC_PAGES), complete: false });
    ...
    const failed = await h.db.query("SELECT run_status,attempt_count,last_succeeded_at,lease_token FROM commerce_connection_sync_states WHERE connection_id=$1", [id]);
    expect(failed.rows[0]).toMatchObject({ run_status: "RETRYING", attempt_count: 1, last_succeeded_at: null, lease_token: null });
    const resumed = await sync();
    expect(resumed.body).toMatchObject({ discoveredCount: 2, cursor: null, complete: true });
  ...
  it("rejects a provider cursor cycle instead of looping or reporting completion", async () => {
    ...
    expect(result.body.error.code).toBe("PROVIDER_CURSOR_INVALID");
  });
});
