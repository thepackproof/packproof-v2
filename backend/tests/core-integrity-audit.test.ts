...
describe("core integrity audit regressions", () => {
  ...
  it("rejects empty and oversized evidence while allowing a valid retry", async () => {
    ...
    const capture = await createCaptureSession(h.db, h.clock, seller, proof.proofId, {
      client: "NATIVE_CAMERA", idempotencyKey: "size-boundary-camera",
    });
    await completeCaptureSession(h.db, h.clock, seller, proof.proofId, capture.id, {
      sha256: originalSha256, byteSize: original.length, contentType: "video/mp4",
    });
    const input = {
      contentType: "video/mp4", evidenceType: "FULFILLMENT_CAPTURE",
      captureSessionId: capture.id, idempotencyKey: "size-boundary",
    };
    const upload = await initializeEvidenceUpload(h.db, h.clock, h.objectStore, seller, proof.proofId, input);
    ...
    const commit = vi.spyOn(h.objectStore, "commitUpload").mockResolvedValueOnce({
      key: "unused-oversized-snapshot", sha256: originalSha256,
    ...
    const retry = await initializeEvidenceUpload(h.db, h.clock, h.objectStore, seller, proof.proofId, input);
    expect(retry.evidenceId).toBe(upload.evidenceId);
    await h.objectStore.put(retry.objectKey, original, "video/mp4");
    const committed = await commitEvidence(h.db, h.clock, h.objectStore, seller, proof.proofId, retry.evidenceId);
    expect(committed).toMatchObject({
      evidenceId: upload.evidenceId, validationStatus: "COMMITTED",
      sha256: originalSha256, byteSize: original.length,
    });
    expect((await h.db.query("SELECT state, evidence_id, expected_sha256 FROM capture_sessions WHERE id=$1", [capture.id])).rows[0]).toMatchObject({
      state: "COMMITTED", evidence_id: upload.evidenceId, expected_sha256: originalSha256,
    });
    expect((await commitEvidence(h.db, h.clock, h.objectStore, seller, proof.proofId, upload.evidenceId)).sha256).toBe(originalSha256);
  });
