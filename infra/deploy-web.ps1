...
if (-not $WebOrigins) { $WebOrigins = $existingOrigins }
$originList = @($WebOrigins -split ',' | ForEach-Object { $_.Trim().TrimEnd('/') } | Where-Object { $_ })
if ($originList -notcontains $WebUrl) { $originList += $WebUrl }
$WebOrigins = ($originList | Select-Object -Unique) -join ','
if ($existingOrigins -ne $WebOrigins) {
  $environment = @()
  $replaced = $false
  foreach ($item in $current.environment) {
    if ($item.name -eq "PACKPROOF_WEB_ORIGINS") {
      $environment += @{ name = "PACKPROOF_WEB_ORIGINS"; value = $WebOrigins }
      $replaced = $true
    } else {
