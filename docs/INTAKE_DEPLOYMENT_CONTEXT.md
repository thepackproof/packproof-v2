...
The first AWS discovery request did not complete, but authenticated control-plane access was subsequently recovered. Current observations on 2026-09-15 supersede that access blocker: account `784514617543`, region `us-east-1`; task definition `packproof-v2-staging-cluster-packproof-v2-staging-api:30`; source `dca1568d403335d5ca3fb585b2db8e82a0227824`; image `sha256:4ae84a49b47e944072e7a8300d2d5202fd922ad85dd394be9bc88900f0c482c9`. This exactly matches the reconciled baseline source tree. Service deployment `ecs-svc/8345445179306842593` is completed with one desired/running task, 512 CPU units and 2048 MiB memory. Native canary deployment uses 5% traffic for three minutes and a three-minute bake, with rollback alarms enabled.

The baseline runs combined API/workers (`PACKPROOF_PROCESS_ROLE` unset), startup migrations disabled, intake flags absent/off, required durable receipts false, Shippo capture shipment worker enabled, and all identifier rollout flags false. **eBay is disabled in the live runtime** with an empty RuName, despite its production application/client and credential references existing. Do not claim functioning automatic eBay sync based on a configured credential reference. The RDS instance is available, with one-day backup retention and latest restorable time `2026-09-15T06:17:23Z` at observation. Redacted reference-only baseline evidence is retained in `docs/intake-deployment-2026-09-15/baseline.json`.

The old runtime rejects any unknown migration ledger row in its schema readiness/startup check. Therefore the additive 067/068 schema requires a compatibility bridge **before** migration: the bridge may accept only these two exact future IDs and checksums while retaining all existing/missing/checksum guards. The pre-migration image `:30` is not a valid post-migration rollback target. Retain the verified bridge image for rollback; never remove schema rows or evidence to make an old binary start.
