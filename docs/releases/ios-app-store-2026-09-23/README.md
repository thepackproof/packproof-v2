...
Distribution-certificate creation reached Apple's CSR upload step. The owner explicitly authorized the previously blocked CSR upload in chat, and the browser accepted the prepared request file. Clicking Continue then produced Apple's “Your session has expired. Please log in.” message before a certificate was issued. The secure authentication handoff returned user takeover, and fresh visible evidence still showed the Apple Developer sign-in form. The next blocker is session renewal; the specific CSR-upload authorization persists. A local CSR and private key were prepared securely outside the repository; no secret values or paths are recorded here. No distribution certificate, provisioning profile, signed IPA, or uploaded build was created.

## Actual release blockers

1. **Distribution signing paused for expired Apple session.** Team verification, identifiers, App Group, capabilities, and the app record are complete. The owner has explicitly authorized the CSR upload, and its prior browser permission blocker is resolved. Renew the Apple Developer session through the secure sign-in handoff, verify authentication, then resume certificate creation with the prepared CSR. Do not request the same upload authorization again. Create both host and extension distribution profiles and configure the required build/APNs credentials securely. Do not store a password/key in source or reuse a simulator archive.
