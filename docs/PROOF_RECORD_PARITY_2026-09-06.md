...
- The security scan identified an insecure camera diagnostic session ID in the recovery baseline. The camera now uses the existing Expo Modules native secure UUID generator, retaining timestamp-based saved-test labels and recovering to the ready state if generation fails. All eight camera diagnostic model tests and native typechecking pass.
