...
4. Only a verified immutable envelope with a readable exact retained object version produces `PRESERVED` and its stable receipt. Both core and policy publishers reread that exact version and require its bytes to match the authenticated envelope. Missing/unversioned or unreadable versions leave publication pending; conflicting bytes halt the ordered journal. A lost response after object publication is recovered from that same object, including its original signature and publication time. Prepared or orphan media alone never establishes an accepted record.
